...
export async function markEventUpdateSeen(eventId: string, updateId: string, userId: string): Promise<boolean> {
  ...
  const rows = await sql`
    ...
    INSERT INTO event_update_notice_receipts (id, notice_id, viewer_user_id, seen_at)
    SELECT ${crypto.randomUUID()}::uuid, accessible_update.id, ${userId}, NOW()
