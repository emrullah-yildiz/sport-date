...
export default async function DiscoveryEventPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return (
    <main className="event-detail-page">
      ...
      {privateLocation ? <section className="accepted-location"><p className="panel-label">Your accepted meeting point</p><h2>{privateLocation.venueName}</h2><p>{privateLocation.address}</p>{privateLocation.instructions ? <small>{privateLocation.instructions}</small> : null}</section> : null}
    </main>
  );
}
