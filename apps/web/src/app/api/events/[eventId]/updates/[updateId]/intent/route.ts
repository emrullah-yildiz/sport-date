...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string; updateId: string }> }) {
  ...
  if (!INTENTS.includes(response as EventUpdateAttendanceIntent)) {
    return NextResponse.json({ error: "Choose a valid attendance response." }, { status: 400 });
  }

  if (!await saveEventUpdateAttendanceIntent(eventId, updateId, viewer.id, response as EventUpdateAttendanceIntent)) {
    return NextResponse.json({ error: "Attendance response could not be saved." }, { status: 409 });
  }
