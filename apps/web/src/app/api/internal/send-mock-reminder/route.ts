...
// given address via the live Gmail credentials, to preview what the T-2h reminder
// looks like. Behind CRON_SECRET. No secret values are ever returned. Remove after use.
export async function GET(request: Request): Promise<Response> {
  ...
  <div style="max-width:520px;margin:0 auto;background:#20262B;border:1px solid rgba(59,234,126,.25);border-radius:16px;padding:28px">
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%"><tr>
      <td style="padding-right:6px;width:50%"><a href="https://keepitup.social" style="display:block;text-align:center;background:#3BEA7E;color:#0c0f10;text-decoration:none;font-weight:800;padding:14px;border-radius:10px">✓ I'm coming</a></td>
      <td style="padding-left:6px;width:50%"><a href="https://keepitup.social" style="display:block;text-align:center;background:transparent;color:#F1F5F3;text-decoration:none;font-weight:700;padding:13px;border:1px solid rgba(255,255,255,.2);border-radius:10px">✗ Can't make it</a></td>
    </tr></table>
    <p style="color:#8A9BA0;font-size:12px;line-height:1.6;margin:20px 0 0">Can't make it? Tapping "Can't make it" frees your spot for someone else — no worries, no penalty. Questions? Just reply to this email (support@keepitup.social).</p>
    <p style="color:#3BEA7E;font-size:12px;font-weight:700;margin:16px 0 0">keepitup.social — meet through movement</p>
    <p style="color:#5a666c;font-size:11px;margin:14px 0 0">This is a preview/mock of the KeepItUp event-reminder email.</p>
  </div></body></html>`;
  ...
  if (sendJson.id) return NextResponse.json({ ok: true, messageId: sendJson.id, to }, { status: 200 });
