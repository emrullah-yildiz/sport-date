  ...
  return (
    <main className="room-page">
      ...
      <section className="room-grid">
        ...
        <article className="room-people"><p className="panel-label">Who has a place</p><h2>{room.participants.length} {room.participants.length === 1 ? "person" : "people"} joining</h2><div>{!room.isHost ? <span><strong>{room.host.firstName}</strong><small>host</small><ReportSafetyControls eventId={room.id} subjectUserId={room.host.userId} subjectName={room.host.firstName} /></span> : null}{room.participants.map((participant, index) => <span key={`${participant.firstName}-${index}`}><strong>{participant.firstName}</strong><small>{participant.skillLevel}</small>{participant.userId !== user.id ? <ReportSafetyControls eventId={room.id} subjectUserId={participant.userId} subjectName={participant.firstName} /> : null}</span>)}</div></article>
      </section>
