    ...
    try {
      ...
      const payload = {
        ...
        location: {
          ...
          private: { venueName: form.get("venueName"), address: form.get("address"), postalCode: form.get("postalCode"), instructions: form.get("instructions"), latitude: null, longitude: null },
        },
      };
  ...
  return (
    <form className="event-form" onSubmit={submit} noValidate ref={formRef}>
      ...
      <section className="event-form-section location-separation" id="section-location" aria-labelledby="section-location-heading">
        ...
        <div className="location-column private-location"><p className="panel-label">Accepted people see</p><h2>The precise meeting point</h2><p>Stored separately and revealed only after acceptance — with one-tap directions.</p><label htmlFor="venueName">Venue name<input {...fieldProps("venueName")} required maxLength={120} placeholder="Court 2" />{fieldMessage("venueName") ? <span id="venueName-error" className="field-error">{fieldMessage("venueName")}</span> : null}</label><label htmlFor="address">Street and number<input {...fieldProps("address")} required maxLength={300} placeholder="Strada Exemplu 12" />{fieldMessage("address") ? <span id="address-error" className="field-error">{fieldMessage("address")}</span> : null}</label><label htmlFor="postalCode">Postal code<input {...fieldProps("postalCode")} required maxLength={20} placeholder="010101" />{fieldMessage("postalCode") ? <span id="postalCode-error" className="field-error">{fieldMessage("postalCode")}</span> : null}</label><label htmlFor="instructions">Arrival instructions<textarea {...fieldProps("instructions")} maxLength={500} rows={3} placeholder="Where to enter, who to ask for, and what to bring." />{fieldMessage("instructions") ? <span id="instructions-error" className="field-error">{fieldMessage("instructions")}</span> : null}</label></div>
      </section>
