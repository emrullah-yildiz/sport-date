...
import PlayMapDiscovery from "./PlayMapDiscovery";
...
export default function ConceptStudio() {
  const [theme, setTheme] = useState<ThemeId>("cards");
  ...
  function navigate(next: ViewId) { setView(next); requestAnimationFrame(() => { contentRef.current?.focus({ preventScroll: true }); contentRef.current?.scrollIntoView({ block: "start" }); }); }
  ...
  return <div className={s.studio}>
    ...
    <div className={`${s.deviceFrame} ${device === "mobile" ? s.mobileFrame : ""}`}><div ref={contentRef} tabIndex={-1} className={s.surface} data-theme={theme} data-view={view}>
      ...
      {view === "discover" && <div className={s.discoveryFilters}><span>What’s your game?</span><div role="group" aria-label="Filter activities">{(["All", ...sports] as const).map(value => <button key={value} aria-pressed={filter === value} onClick={() => setFilter(value)}>{value !== "All" && <SportGlyph sport={value} size={17} />}{value === "All" ? "Anything goes" : value}</button>)}</div></div>}
      <main>{view === "discover" ? theme === "cards" ? <CardsDiscovery items={items} onEvent={openEvent} onProfile={() => navigate("profile")} sport={sport} setSport={setSport} scenario={scenario} /> : theme === "clubhouse" ? <ClubhouseDiscovery events={items} onEvent={openEvent} onProfile={() => navigate("profile")} onProgress={() => navigate("progress")} /> : <PlayMapDiscovery events={items} onEvent={openEvent} onProfile={() => navigate("profile")} onProgress={() => navigate("progress")} /> : view === "profile" ? <ProfileView finish={availableFinish} theme={theme} sport={sport} setSport={setSport} scenario={scenario} onEvent={() => openEvent(events.find(event => event.sport === sport)!)} /> : view === "event" ? <EventView event={selectedEvent} status={status} onRequest={() => advance("request")} onProfile={() => navigate("profile")} onDiscover={() => navigate("discover")} onProgress={() => navigate("progress")} /> : <ProgressView theme={theme} count={progress} finish={availableFinish} onFinish={count => { setSelectedFinish(count); navigate("profile"); }} onDiscover={() => navigate("discover")} />}</main>
