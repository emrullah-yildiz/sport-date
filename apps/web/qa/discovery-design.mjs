...
const mocks = {
  ...
  '@/lib/events': `export async function getDiscoverableEvents(){return window.fixtureEvents}`,
  ...
  'next/navigation': `export function redirect(){throw Error('Unexpected redirect')}export function useRouter(){return {push(){},refresh(){}}}export function usePathname(){return '/discover'}export function useSearchParams(){return new URLSearchParams()}`,
  'server-only': '',
};
const result = await build({configFile:false,root,logLevel:'error',oxc:{jsx:{runtime:'automatic'}},resolve:{alias:{'@':path.join(root,'src')}},plugins:[{name:'discovery-fixture',enforce:'pre',resolveId(id){const key=Object.keys(mocks).find(k=>id===k || (k.startsWith('@/') && id.replaceAll('\\','/')===path.join(root,'src',k.slice(2)).replaceAll('\\','/')));if(key)return '\0mock:'+key;if(id.endsWith('virtual:discovery'))return '\0entry';},load(id){if(id.startsWith('\0mock:'))return mocks[id.slice(6)];if(id==='\0entry')return `import React from 'react';import{createRoot}from'react-dom/client';import Page from '@/app/discover/page';const root=createRoot(document.getElementById('root'));window.renderFixture=async()=>root.render(await Page({searchParams:Promise.resolve(window.fixtureParams||{})}));window.renderFixture();`;}}],build:{write:false,minify:false,lib:{entry:'virtual:discovery',name:'DiscoveryFixture',formats:['iife']}},define:{'process.env.NODE_ENV':JSON.stringify('production')}});
...
try {
  for(const [width,reducedMotion] of [[390,'no-preference'],[390,'reduce'],[1280,'no-preference']]) {
    ...
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
    assert.deepEqual(errors,[]);await page.close();console.log(`PASS discovery ${width}px ${reducedMotion}: finite keyboard browse, grid, empty/single, filters, no overflow or requests`);
  }
