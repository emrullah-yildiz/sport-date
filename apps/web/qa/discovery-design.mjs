...
try {
  for(const [width,reducedMotion] of [[390,'no-preference'],[390,'reduce'],[1280,'no-preference']]) {
    ...
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
    await page.evaluate(()=>{window.fixtureEditor=true;window.editProfile={firstName:'Ana',lastName:'Example',location:'Bucharest',bio:'A relaxed game',languages:['English'],seeking:'friendship',sports:[{name:'Tennis',skillLevel:'beginner',frequency:'casual'}],prompts:[],gender:null,genderSelfDescribe:'',genderVisible:false,sexualOrientation:null,orientationSelfDescribe:'',orientationVisible:false,orientationConsent:false};window.fetch=async(url,init)=>{if(url!=='/api/account/profile'||init?.method!=='PATCH')throw Error('Unexpected editor request');return new Promise(resolve=>{window.resolveSave=()=>resolve(new Response(JSON.stringify({})));});};return window.renderFixture()});
    await page.locator('#edit-profile > summary').click();
    await page.locator('#edit-profile-bio').fill('A new introduction');
    await page.locator('#edit-profile button[type="submit"]').click();
    assert.equal(await page.locator('#edit-profile-bio').isDisabled(),true);
    assert.equal(await page.locator('#edit-profile input:enabled, #edit-profile select:enabled, #edit-profile textarea:enabled').count(),0);
    await page.evaluate(()=>window.resolveSave());
    await page.getByText('Profile updated.',{exact:true}).waitFor();
    await page.locator('#edit-profile-bio').fill('Another unsaved introduction');
