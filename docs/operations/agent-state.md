...
1. A non-DB engineering/a11y slice if a clean one exists: encode the roadmap's "never reward swipes/streaks/time-spent" principle as an explicit *tested* domain guard/lint (the progression rule is implemented and tested for what it *does* reward; a guard asserting it never rewards the prohibited signals would make the principle enforceable against regressions). Needs no DB, infra, or owner decision; keep typecheck and the suite green.
2. Build on the legal draft set with adjacent counsel-prep artifacts that are still autonomous: a draft Safety & Community Guidelines document (referenced by the Terms draft but not yet written), and/or a draft cookie/tracking statement once the analytics boundary is decided — all labelled DRAFT-for-counsel and grounded in implemented behaviour.
