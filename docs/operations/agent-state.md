...
Threat-modeled the implemented email-verification and password-reset flows in `docs/security/authentication.md` (new "Email verification and password reset threat model" section), grounded in the real code rather than the design doc. Each threat is written as threat -> control -> verification (citing the specific unit or route test) -> residual risk / tracked gate: 256-bit CSPRNG opaque tokens with SHA-256-hash-only storage and flow-prefixed validation; single-use consumption with sibling invalidation; 24h verification / 60m reset expiry with no side effect on expiry; reset-request enumeration neutrality (identical 202 across exists/missing/db-down/throw); the reset->browser-delete + mobile-revoke session boundary; SHA-256 requester-IP hashing (null on unknown); per-flow 429 throttles (resend/reset-request/confirm) that preserve the neutral message; and the default-disabled, provider-gated, canonical-origin delivery seam. The honest-claim boundary is restated explicitly: verification proves inbox control only, not identity/age/safety. Owner/counsel gates (provider enablement, IP-hash retention) and the engineering gate (real-PostgreSQL integration test) are flagged. Docs-only; suite unchanged at 142.
...
1. Add an end-to-end (or DB-integration) test of the verification/reset persistence against an isolated PostgreSQL instance once the test-database gate is met, so token single-use, sibling invalidation, and session revocation are asserted against real SQL rather than recorded statements. (Engineering gate: an isolated PostgreSQL test instance must exist; autonomous once it does.)
2. Reconcile and consolidate the enumeration-resistance story across login (dummy-hash timing) and the reset request (constant response, non-constant time) into one documented account-enumeration posture, and decide whether constant-time padding is warranted before the edge rate-limit gate lands.
3. Consolidate the four authentication/abuse "launch gates" into a single owner-facing escalation packet (edge rate limiting, email provider, counsel review of retention/lawful basis, moderation owner) so the owner can unblock several roadmap items in one decision. Most remaining now-bucket items are owner/counsel/infrastructure gated rather than codeable.
