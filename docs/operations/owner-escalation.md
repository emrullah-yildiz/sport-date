...
- **Status (2026-06-30): the legal set is now OWNER-FINALIZED, but Gate 2 is NOT cleared.** On the owner's "legal looks OK, finalize it" instruction, the four drafts were promoted to the canonical, owner-approved operational documents in `docs/legal/` (`privacy-notice.md`, `terms-of-service.md`, `safety-community-guidelines.md`, `consent-copy.md`; index `docs/legal/README.md`). Each carries an honest "PUBLISHED — owner-approved as of 2026-06-30" header that **explicitly does not claim lawyer review or approval** and states that qualified-EU-counsel review remains recommended before relying on them in a dispute — so the AGENTS.md rule against representing legal advice as counsel is preserved. The `[OWNER:]`/`[COUNSEL:]` markers were resolved onto the working assumptions (name "Sport Date", EU/GDPR baseline, Bucharest hypothesis, implemented retention) and the documents now use the actual implemented retention behaviour where the code fixes a period. **Two things still block real-user reliance and a true Gate 2 close:** (1) qualified EU counsel still needs to review the substantive legal judgements flagged inline (Article 9 position, lawful bases, retention durations, transfers, DSA scope, the liability section, age assurance); and (2) the owner-only facts marked `[OWNER TO CONFIRM: …]` — legal entity name & registered address, a monitored privacy/contact + safety/abuse email, the DPO/representative if any, the processor list + DPAs, a named safety owner (Gate 5), real Terms/Privacy/app links + an enabled email provider (Gate 4), the backup schedule, and the final launch jurisdiction — must be supplied. Both are consolidated per document and in `docs/legal/README.md`.
