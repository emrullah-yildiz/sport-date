...
- the server records that eligibility at reflection time.

Elapsed time alone never creates progress. Requests, acceptances, host skips, profile browsing, reports, daily login, and screen time never create progress. A host cannot advance with an empty event. Later cancellation or safety access changes do not silently rewrite whether the event qualified at reflection time, while correcting the reflection from `attended` updates the visible count.
