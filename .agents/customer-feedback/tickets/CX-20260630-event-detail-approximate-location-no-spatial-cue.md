...
- `2026-07-02` - Implemented (Experience Build Agent), status `implemented`, commit `29715d8` (pushed origin/main; no migration). Added a self-contained SVG **approximate-area** cue to the "Before acceptance" panel — a soft diffuse zone under a dashed ring that reads as an AREA, not a pin — beside the existing honest text, plus an optional calm distance band ("About N km from your area"). **How privacy is preserved (coarse-only, no exact venue, no external tile host):** the cue derives ONLY from data discovery already exposes — the event's already-public approximate 0.1° grid coordinate (or, when unset, an offline city-centre geocode), the area/city labels, and the VIEWER's own free-text profile area — reusing `discovery-geo`'s `coarseCentreForEvent`/`coarseCentreForArea`/`distanceKm`. The exact venue is never read on this surface (it is not joined into `getDiscoverableEvent`). The SVG has no axes/scale/coordinates; the zone placement is seeded from the coarse cell only (no sub-cell precision survives) and the distance is snapped to a wide band (area-to-area), so neither party's precise point can leak or be reverse-engineered. It is inline SVG drawn from design tokens — no external tile host (CSP-safe). The visual is decorative (`role="img"` + text-equivalent `aria-label`) and never required to complete the journey; with SVG/images off the honest disclosure text still fully communicates the approximate location. New `lib/approximate-location.ts` (+ `.test.ts`, 19 cases pinning the privacy invariant / no precise coordinate or venue leak / coarse distance banding / region-not-point placement); `components/ApproximateAreaMap.tsx` (server component, static, reduced-motion safe); anthracite+neon `.approx-area*` tokens in `globals.css`, AA, 44px n/a (no new interactive target), visible focus via global fallback, overflow-safe at 375/1280. **Checks (apps/web):** typecheck pass, lint 0 errors, test 707 (695 pass / 12 skipped) incl. ethical-energy-guardrails GREEN, production build pass. Handing back for independent retest.
