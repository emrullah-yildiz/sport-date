...
- [x] **≥3 threshold before any aggregate shows:** below 3 ratings the recipient sees a calm "not enough ratings yet" state; at ≥3 they see an **aggregate average** of their own received ratings; individual ratings and who-gave-what are never shown.
- [x] **Recipient-visible only:** the aggregate/average/count is shown only to the recipient — never to other members, on discovery, on member-to-member profiles, or on any public profile in this slice.
...
- [x] Abuse-resistance: rate-limited, one-per-pair-per-event, edit-window-then-lock consistent with the existing slice; a single revenge rating cannot define someone (guaranteed by the ≥3 threshold + double-blind).
- [x] Tests cover: the co-attendance gate, the ≥3 aggregate threshold, the double-blind reveal rule, and the no-public/other-member exposure invariant; repository checks pass.
