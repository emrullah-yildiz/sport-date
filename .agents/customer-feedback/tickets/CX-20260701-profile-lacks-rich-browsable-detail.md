...
- 2026-07-01 - Independently retested by experience-design-explorer (logged in ONCE as pooled host-A, session reused; real Chromium under reducedMotion:reduce at 1280 + 375). **VERIFIED** — 37/37 member-checkable checks pass. Humane sections all present (Intro, Looking for with "first-class" seeking-as-equals copy, Languages chips, On the field with human skill/frequency phrases e.g. "Plays at an intermediate level · most weeks", In their words). Personality prompts ADD (2 prompts → render in "In their words"), EDIT (answer replaced, old removed), and REMOVE (calm "No prompts answered yet…" empty state) all persist across reload. Curated-only + bounds enforced server-side: >3 prompts → 400, >140-char answer → 400, uncurated free-text prompt ("My phone number is") → 400. GDPR `/api/account/export` includes `personalityPrompts` array with the added answers. Name-forward hero (h1 = "Host Alpha", exactly one h1, single eyebrow token 8.08 contrast) and two-tier action strip (lime Host / ink Discover+Your events primary; quiet Safety/legal secondary group) preserved. No attractiveness/score/popularity strings; no precise location/contact leak; primary targets 49–59px (≥44); AA eyebrow contrast 8.08; no horizontal overflow at either width. Test account mutations reverted after retest. Migration `020` + `validateProfileUpdate` + export wiring source-confirmed. Original implementation note below retained.
