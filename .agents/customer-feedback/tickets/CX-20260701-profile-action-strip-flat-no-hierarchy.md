...
- 2026-07-01 - Independently retested by Experience & Design Explorer WITHOUT reading the implementation notes first — logged in ONCE as pooled `host-A` (single `/api/auth/login`, reused for both viewports), real Chromium at 1280 and 375px. **Every acceptance criterion passes → `verified`.** Live DOM/computed-style probe: two `<nav>` groups — primary `aria-label="Your main actions"` (top 349/533px) holds exactly the 3 lead actions **Host an event** (bg lime `rgb(201,244,88)`, ink text, aria "create a new game"), **Discover events** and **Your events** (both ink `rgb(23,36,29)`, white text), all 17px/weight-900, heights 49–59px; secondary `aria-label="Safety, legal, and support"` (top 424/736px, below the primary group at both widths — `primaryAboveSecondary=true`) is a real `<ul>` of 4 quiet **outlined** links (transparent bg, muted `rgb(102,113,105)`, 14px, 44px each): Safety center, Trust **preview** (aria "— pre-launch"), Privacy Notice **preview** (aria "— pre-launch"), Share feedback — all reachable and honestly labelled, none hidden. Lime appears on Host only. Name-forward hero preserved (h1 = the member's name "Riley HostA" at 60px/36px). No horizontal overflow at either width (`docScrollW==docClientW`). 375px screenshot confirms the 3 primaries lead and the SAFETY/LEGAL/SUPPORT group reads as clearly secondary but reachable. Anti-dark-pattern guardrails intact (safety/privacy access preserved, preview labels honest). Status `implemented` → `verified`.
