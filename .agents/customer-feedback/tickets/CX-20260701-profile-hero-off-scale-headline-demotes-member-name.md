...
- 2026-07-01 - experience-build-agent implemented (commit `40e53ec`); status `implemented`. Changes: `.profile-hero h1` now holds `{firstName} {lastName}` at `--fs-h1` with standard `-.02em`/`1.05` (removed bespoke `clamp(48,7vw,82)`/`-.065em`/`.96`); marketing sentence demoted to a body-scale lede `<p>`; About panel h2 now leads with the member's stated city (removed name duplication); `.profile-panel h2` on `--fs-h2`; `.profile-sport span` on `--fs-small` (14px); removed the duplicate `.eyebrow` rule in the legacy `.hero` block so the single label token (weight 700 / `--ink-soft` / `.14em`) is authoritative. Checks: typecheck pass, lint pass (0 errors; only pre-existing warning is in untracked `qa/full-flows.mjs`), test 165 pass / 12 skipped. Live: logged in as pooled `host-A`, viewed `/profile` at 1280 & 375 — name leads at 60px / 36px on-scale, panel h2 44px / 28px, eyebrow single token, sport meta 14px, no overflow. No new personal data / precise location; heading order, focus, reduced-motion, AA contrast intact. Ready for independent Explorer retest.
- 2026-07-01 - Independently retested LIVE by experience-design-explorer WITHOUT reading the diff first (logged in as pooled `seeker-B` via `/api/auth/login`; host-A's per-email auth budget was exhausted (429 `browser-auth-email`, retry-after 518s) so switched pooled account per the on-429 fallback; real Chromium, `/profile` at 1280 & 375). Measured computed styles: **RESULT = `verified`, every AC passes.** (1) h1 = the member's NAME ("Sam SeekerB"), on `--fs-h1` scale — 60px @1280 / 36px @375, letter-spacing -1.2px/-0.72px (≈-.02em), weight 700 — no bespoke off-scale clamp. (2) Marketing sentence ("Ready when the right game appears…") is now a subordinate lede `<p>` at 17px body / weight 400 / muted, clearly below the name (screenshot confirms the name is the visual protagonist). (3) `.profile-panel h2` on `--fs-h2` — 44px @1280 / 28px @375 (scales, not floor-pinned). (4) `.profile-sport span` = 14px (`--fs-small`), no longer sub-token. (5) Exactly **1 h1** on the page; heading order h1→h2… clean (single-h1). (6) `.eyebrow` resolves to one consistent token (12px / weight 700 / 1.68px tracking / rgb(42,79,61)) with no source-order flip. (7) **No overflow** at either width (doc 1280==1280 / 375==375; hero 1120==1120 / 335==335). (8) Contrast AA: h1 14.13, panelH2 16.07, eyebrow 8.08 all pass; the body-lede and sport-meta use the app-wide `--muted` (#667169 Sage) at ~4.47 on cream / 5.08 on the lighter hero gradient — this is the systemic secondary-text token (pre-existing, unchanged by this ticket, which only enlarged the meta from 12→14px) and reads AA against the lighter part of the hero; flagged for the separate systemic-Sage-contrast concern, not a regression here. No new personal data / precise location; reduced-motion & focus unaffected. Redacted synthetic account only.
