...
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. b5cdddd). All 6 properties confirmed against code + tests: (1) upsert `ON CONFLICT (event_id,requester_user_id) DO UPDATE SET status=pending,...clears cancelled_at/responded_at/last_skipped_at/skip_count/exit_reason/exit_note,refresh introduction/requested_at WHERE join_requests.status='cancelled'` — WHERE guard scopes reopen to the member's OWN cancelled row (accepted/declined/pending never reopened); (2) all join guards preserved in INSERT...SELECT WHERE (published+future, host-exclusion, age, skill, language, capacity, mutual user_blocks) → closed/full/blocked/ineligible → empty RETURNING → null → route 409; (3) RETURNING returns real row id (test asserts, not a fresh uuid); (4) reliability cooldown preserved + ordered — getMemberReliabilityStanding/paused runs BEFORE upsert (route 423 Locked), reliability.ts untouched, expired pause proceeds; (5) UI cancelled branch → "Request a place again" returns join form (status=null), focus to note textarea (no body-stranding), non-punitive copy + "skip counts stay private"; (6) tests capture real SQL — reopen-to-pending asserts DO UPDATE/WHERE cancelled clause, real-id, guard→null, active-cooldown→paused with queries length 1 and NOT matching ON CONFLICT (upsert never ran), expired→proceeds. No schema change (005 already has the UNIQUE target). Checks the Tester ran itself: typecheck PASS, lint PASS, test 546 passed/12 skipped, prod build PASS. Orchestrator applied `verified` in main tree and archived.
