...
- [x] Each upcoming host card on `/hosting` shows a calm count of pending join requests (with a sensible "No requests yet" zero state) and the places-filled-vs-capacity state. (`page.tsx:35-57`; `summarizeHostCoordination` events.ts:435 — "No requests yet" / "N of M places filled".)
- [x] The pending-requests element links to (or is) a one-click path to act on those requests on the event page. (pending>0 renders a `<Link href="/events/{id}">` with an aria-label; zero state is plain text — page.tsx:40-48.)
- [x] Only aggregate counts appear on the hub — no requester names, skip counts, attractiveness/popularity metrics, or precise locations are exposed (anti-dark-pattern + privacy honoured). (`HostCoordination` carries only three scalars; authz test pins `hostCoordination: null` for joined-not-hosted rows; no skip/name/email in the query or markup.)
- [x] Copy is host-toned and truthful, matching implemented behaviour; no urgency/scarcity pressure (coral used sparingly if at all). (Waiting item uses `--lime`, never coral — globals.css:584; unit tests pin calm phrasing.)
- [x] Loading, empty (no events, and events-with-no-requests), and failure states remain coherent with the new fields. (Empty/section-empty branches unchanged; `coordination` block guards on `hostCoordination` truthiness so zero-request cards still render a calm "No requests yet".)
- [x] Mobile 375 and web layouts stay free of overflow; the count is keyboard-reachable, screen-reader-named, 44px target where interactive, reduced-motion safe, on-brand (Ink/Cream/Lime/Coral/Sage). (`.hosting-coordination-item` flex-wraps; grid collapses to 1col @850px; pending link `min-height:44px`; shared `.hosting-page a:focus-visible` ring; skeleton animation disabled under reduced-motion.)
- [x] Relevant automated tests and repository checks pass (extend the `selectHostedEvents`/summary tests for the new count derivation). (events.test.ts 16/16 pass, incl. host-only filter, carry-through, upcoming/past, and the null-for-joined authz contract + zero/singular/plural/full copy.)
...
- 2026-07-01 - **Verified** by Experience & Design Explorer (retest, resolved in ONE step per the no-poll handshake, without reading the implementer explanation first). Source-confirmed every AC: `getMemberEventSummaries` adds two additive read-only scalar subqueries (accepted seats via `event_participants`, pending via `join_requests WHERE status='pending'`) + `events.capacity`, computed for all rows but attached as `hostCoordination` ONLY when `events.host_user_id = userId` (authz — a joined-not-hosted row gets `null`, pinned by the "does not attach coordination counts to an event the member only joined" test). `page.tsx` renders per-upcoming-card a calm aggregate pending count linking to `/events/{id}` (accept/decline) plus "N of M places filled" / "All places filled"; past cards show places + "Requests are closed"; NO requester identity/skip/location on the hub (aggregate scalars only). On-brand: waiting item uses `--lime` (positive), never coral/scarcity; 44px pending link; flex-wrap + 1-col@850px guard overflow; reduced-motion-safe skeleton; shared focus ring. `events.test.ts` 16/16 pass. Live: the core hub path (zero-request "No requests yet" / "0 of N places filled" from real DB counts, no requester data) was observed live at build commit `ecafae6` as pooled host-A; this retest's live re-drive hit the per-IP browser-auth 429 (not polled per policy). Sole source-only sub-branch: the `pending>0` rendering (needs a second member's live request; rate-limited) — fully covered by the `summarizeHostCoordination` singular/plural unit cases + the null-for-joined authz test. Every criterion passes → `verified`. Each upcoming host card now shows an aggregate pending-request count ("N people waiting for your reply" / "No requests yet") linking to the event's accept/decline flow, plus places-filled-vs-capacity ("N of M places filled" / "All places filled"); past cards mark requests closed. Data path is additive with NO migration: `getMemberEventSummaries` gains two read-only scalar subqueries (accepted seats via `event_participants`, pending via `join_requests`) + `events.capacity`; new `HostCoordination` is attached only when the viewer hosts the event (authz-tested — no other host's counts, no requester identity/skip/location on the hub). New pure helper `summarizeHostCoordination` unit-tested (singular/plural/full). Checks: typecheck pass, lint pass (only pre-existing untouched `qa/full-flows.mjs` warning), test pass (267), production build pass (`/hosting` dynamic). Live-verified as pooled host-A: hub rendered "No requests yet" / "0 of N places filled" from real DB counts with no requester data exposed (seeker join-request end-to-end blocked by login rate-limit this pass; pending>0 branch covered by the shared helper's unit tests). Status `implemented` — awaiting independent Explorer retest. Committed LOCALLY only (prod push held).
