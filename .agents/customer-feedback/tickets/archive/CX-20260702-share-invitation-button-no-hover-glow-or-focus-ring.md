...
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. a9159ad). Fix is UNSCOPED on `.share-event-button` (globals.css:732): `:hover { box-shadow: var(--glow-accent) }` + `:focus-visible { outline: 3px solid var(--focus); outline-offset: 2px }` + disabled resets `box-shadow:none`. Grep confirms exactly two render sites (`.host-published` It's-live panel events/[eventId]/page.tsx:128, and `.room-people-empty` room/page.tsx:171) and both inherit the unscoped rules — none uncovered; old `.room-people-empty`-scoped glow removed (that instance keeps only its blue hover background). `--glow-accent` is pure box-shadow (no transform) → persists under reduced-motion; disabled = no glow; tokens only, AA unaffected. Colour: unified on GREEN (positive/primary CTA) per orchestrator directive rather than the ticket-AC blue — deliberate, consistent, single source of truth (meets the "cannot drift again" requirement); acceptable, not a reopen. Checks the Tester ran itself: typecheck PASS, lint PASS, test 532 passed/12 skipped, prod build PASS. Orchestrator applied `verified` in main tree and archived. Closes the app-wide "every button glows on hover" standard.
