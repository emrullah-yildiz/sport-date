...
- 2026-07-02 - test - **VERIFIED** (independent retest). Source: `PlusBilling.tsx` returns `null` when `!billingConfigured` (fail-closed — no price, no pay action, surface fully hidden when dormant); `profile/page.tsx` computes `billingConfigured = isBillingConfigured()` and `memberIsPlus = isPlus(user)` server-side and passes them in (client never reads keys). Free member → honest "€6.99/month · VAT included" + allowed perks only (advanced filters, richer customization/prompts, own meet/reflection history, quiet supporter badge "never a ranking", early access to non-safety features) + "Safety and joining games are always free" + photos capped at 6 for everyone + Upgrade→/api/billing/checkout; Plus member → "You're a supporter" + "Manage or cancel"→/api/billing/portal. `api/billing/portal/route.ts` requires auth + same-origin CSRF check, FAILS CLOSED with calm 503 before any Stripe call when `!isBillingConfigured()`, delegates cancel to Stripe's hosted portal (no custom cancel/guilt loop). No forbidden perk (boosts/likes/priority/"who viewed you"/extra photos/popularity), no urgency/scarcity/countdown. Accessible section w/ aria-labelledby, `role="status"` messages. No secret in client. Repo checks pass (typecheck/lint/test 442 pass/build). AC all met. Status `implemented` → `verified`.
