...
- 2026-07-02 - Implemented by Experience Build Agent; status `implemented`. Recovery path added on both dead-end surfaces. Reset-password (`apps/web/src/components/PasswordResetConfirmCard.tsx`): every dead-end (missing token, malformed token, and valid-format token rejected on submit) now renders the request form inline via `ForgotPasswordPanel` (open, trigger label "Send me a new reset link"), reusing `/api/auth/password-reset/request` so the anti-enumeration response is identical to the login panel; "Back to sign in" kept as secondary; alert prose updated to point at the inline form. `ForgotPasswordPanel` (`apps/web/src/components/ForgotPasswordPanel.tsx`) gained `defaultOpen` + `triggerLabel` props (stays a collapsible disclosure). Verify-email (`apps/web/src/components/EmailVerificationConfirmCard.tsx`): every non-success state now leads with a precise deep link "Get a new verification link" → `/profile#account-security` (new `id` anchor added on the Account panel in `apps/web/src/app/profile/page.tsx`), with an honest `.auth-flow-hint` note that resending is auth-gated ("you may be asked to sign in first"); "Sign in" and "Create account" kept as secondary. Styling: new `.auth-flow-hint` in `apps/web/src/app/globals.css` uses `--text-muted` (AA). Security posture preserved: token validation/expiry unchanged, anti-enumeration copy reused verbatim, honest verify disclaimer kept, no precise-location or account-existence exposure, no dark patterns. Tests: added `apps/web/src/components/PasswordResetConfirmCard.test.tsx` and extended `EmailVerificationConfirmCard.test.tsx` (recovery action + secondary options + anti-enumeration + auth-gated note). Checks (apps/web): typecheck pass, lint pass (only pre-existing warnings in untouched files), unit tests 530 pass / 12 skipped, prod build pass; live-probed both pages at localhost:3000 (recovery controls present, deep-link href present). AA confirmed. No migration. Commit `43863b5`.
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. 43863b5). Reset dead-ends (`deadEnd = !completed && (!token || !tokenValid || Boolean(error))`) cover missing/malformed/server-rejected token; each renders `<ForgotPasswordPanel defaultOpen triggerLabel="Send me a new reset link" />` inline wired to `/api/auth/password-reset/request`, "Back to sign in" secondary. Verify dead-ends lead with `/profile#account-security` "Get a new verification link" (anchor confirmed at profile/page.tsx:165), sign-in/create-account secondary, honest auth-gated `.auth-flow-hint` note. SECURITY: token gates/confirm routes untouched; anti-enumeration intact (request route returns identical GENERIC_SUCCESS_MESSAGE in all branches — success/db-unconfigured/error); no account-existence or PII leak; verify disclaimer preserved. a11y/AA: `.auth-flow-hint` --text-muted (7.13 AAA), tokens only, role=alert/status + live region kept, autocomplete=new-password intact. Tests genuine (recovery path + anti-enumeration), not tautologies. Checks the Tester ran itself: typecheck PASS, lint PASS, test 530 passed/12 skipped, prod build PASS. Orchestrator applied `verified` in main tree and archived.
