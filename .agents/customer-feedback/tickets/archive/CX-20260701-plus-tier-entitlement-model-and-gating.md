...
- [x] The **free tier is fully usable**, including **all safety features and all core participation**; nothing a free member needs is gated. Safety = report, block, leave, emergency guidance, approximate-location privacy, Safety Center, moderation, appeals. Core = discover, request, cancel, attend, host baseline, message, sign out, basic profile, reflection, up to 6 photos (photos capped at 6 **for everyone**, no per-tier photo advantage).
- [x] A per-member entitlement exists in the data model, **defaulting all members to free** (additive migration only); no billing/subscription/payment/price tables added by this ticket.
- [x] A single server-side `isPlus(user)` / `canUse(user, capability)` helper is the **one place** Plus checks live, and **fails closed** (defaults to free / least-privilege) on unknown/missing/`null`/expired tier.
- [x] The helper is covered by tests: free denied Plus-only conveniences; Plus allowed; **safety/core capabilities are never routed through the gate**; fail-closed on unknown/missing/expired tier; expired `plus_until` resolves to free.
- [x] A **regression guard** asserts safety + core-participation capabilities are not behind the Plus gate (so a future change that gates one fails the suite).
- [x] `SessionUser` (or the server user shape) carries the tier, resolved server-side, defaulting free when absent.
- [x] **No charging, no processor, no secret, no price, and no "you are subscribed" claim** is added anywhere. If any Plus affordance placeholder is shown, it is honest (describes the convenience, notes it is a Plus perk) with no charge/subscribe action.
- [x] **Forbidden-perks guardrail honored:** the gate is only ever wired to allowed convenience/richness capabilities; it is never wired to paid boosts/likes/priority visibility, paid access to people, "see who rated/skipped/viewed you", unlimited/per-tier photos, or any attractiveness/popularity mechanic.
- [x] **Cancel-easy note:** entitlement clears cleanly to free on expiry/cancellation (the actual cancel path lives in the Stripe + billing-UI tickets); a lapsed member is never left in a broken half-Plus state.
- [x] Relevant automated tests and repository checks pass (`npm run typecheck`, `npm run lint`, web + domain test suites).
...
- 2026-07-01 - test (Claude Opus 4.8): **VERIFIED**. Repo checks all pass (typecheck ✓, lint ✓ — sole warning is untouched `qa/full-flows.mjs`, web test 368/12 skip, domain 156, production `npm run build` ✓). SOURCE: migration 025 additive nullable `users.plus_until`; pure fail-closed gating in `packages/domain/src/entitlements.ts` (`isPlus` true only while `plusUntil > now`; null/missing/malformed/expired/invalid-`now` → FREE); server seam `apps/web/src/lib/entitlements.ts` fails closed on null user; `SessionUser` carries `plusUntil` resolved in BOTH `getCurrentUser` and `getMobileSession` (`normalizePlusUntil` coerces unparseable → null). TYPE-LEVEL SPLIT confirmed: safety/core live in a separate `ALWAYS_FREE_CAPABILITIES` set, disjoint from `PlusCapability`; `entitlements.test.ts` regression guard asserts by-name that every safety (report/block/leave/emergency/approx-location/safety-center/moderation/appeals) and core capability is never Plus-gated AND that no forbidden perk (boost/priority/see-who-viewed/paid-likes/unlimited-photos/attractiveness/popularity) is gate-able. No billing/charge/subscription/price/"you are subscribed" claim added; no real perk paywalled. LIVE (dev server, single pooled login as host-A): `/discover /profile /hosting /safety /events/new /feedback` all 200; safety block endpoint reaches domain validation (400 "Choose a valid member to block.") not a paywall; zero `402`/payment-required in app source. `getCurrentUser` reads `plus_until` without breaking any core page. Migration additive + already applied to prod per builder note. All acceptance criteria ticked → Status `verified`.
