...
- 2026-07-01 - Experience & Design Explorer: independently retested from the member scenario WITHOUT reading the diff first, live in Chromium (reduced-motion) as pooled `host-A` (logged in ONCE via `/api/auth/login`, session reused). VERIFIED. AccountMenu present on ALL 8 authenticated surfaces at 1280 AND 375: /discover, /hosting, /events/[id], /events/[id]/room, /events/new, /feedback, /safety, /profile (menuTrigger=1 each), no horizontal overflow at either width. Accessible disclosure confirmed: trigger `aria-haspopup=menu`, `aria-expanded` toggles false→true→false, `aria-controls` set, panel `role=menu` with `role=menuitem` items = ["Your profile","Switch account","Sign out"]; focus moves INTO the menu on open; Escape closes AND returns focus to trigger; outside-click closes; trigger + all three items exactly 44px; visible focus ring defined. Sign out → landed on /login, and a subsequent /discover 307-redirected to /login, i.e. the session was actually cleared. Both Sign out + Switch account POST /api/auth/logout (source-confirmed). Status `verified`.
