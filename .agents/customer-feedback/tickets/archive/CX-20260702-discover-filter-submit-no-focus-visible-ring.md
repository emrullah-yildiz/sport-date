...
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. e12af3a). `git show e12af3a` = single one-line append: `.discover-filters button:focus-visible { outline: 3px solid var(--focus); outline-offset: 2px }` co-located with the base rule (which had NO :focus-visible before — refutes the assumption a ring existed). Token `--focus` #3BEA7E on `--bg` rgb(32,38,43) ≥3:1 (WCAG 1.4.11/2.4.7). Target confirmed: discover/page.tsx:163 the only classless <button type=submit> in form.discover-filters. Outline+offset only → no layout shift; hover glow + :disabled reset untouched. Checks the Tester ran: typecheck PASS, lint 0 errors, test 570 passed/12 skipped, prod build PASS. FOLLOW-UP: a systemic global `:focus-visible` fallback would close the whole focus-ring-gap class (user-sim has found several one-by-one) → filed CX-20260702-global-focus-visible-fallback-for-interactive-elements. Orchestrator archived.
