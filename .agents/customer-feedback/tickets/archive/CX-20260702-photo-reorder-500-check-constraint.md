...
- 2026-07-02 - test - **VERIFIED** (independent retest; live Blob path source-only, no BLOB_READ_WRITE_TOKEN in env). Source: `photos.ts` `reorderProfilePhotos` now runs ONE set-based `UPDATE ... FROM unnest(ids::uuid[], positions::int[])` scoped to `user_id` inside `sql.transaction([...])` — the old two-phase `position + MAX_PROFILE_PHOTOS` (+6) that violated `CHECK (position < 6)` (23514) is GONE; final positions are 0..n-1 so no CHECK violation is possible, and it never touches `is_primary` (so the one-primary partial index can't trip). Migration `028_profile_photos_position_deferrable.sql` is additive: drops the non-deferrable unique index and re-adds `UNIQUE (user_id, position) DEFERRABLE INITIALLY DEFERRED` — same guarantee, checked at COMMIT so the mid-permutation transient collision is legal. Unit test (`photos.test.ts` "reorders 2+ photos in one collision-free set-based statement (CX-20260702)") reproduces the old 500 setup (3 photos), asserts exactly one write, no `position + `, contains `unnest` + `SET position = v.pos`, no `is_primary`, and args `[["c","a","b"],[0,1,2],"42"]`; a permutation-validation test guards non-permutations. Repo checks pass: typecheck ✓, lint ✓, test 442 pass/12 skip ✓, production build ✓. No regression to upload/set-primary/delete/purge. AC all met (live 200-persist branch source-verified). Status `implemented` → `verified`.
