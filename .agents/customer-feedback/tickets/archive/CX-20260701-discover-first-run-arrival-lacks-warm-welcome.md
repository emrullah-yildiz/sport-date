...
- [x] The /discover header greets the member warmly and by their own first name (e.g. "Welcome back, {firstName}") using only already-available data; the generic pre-signup marketing sentence is not the hero H1.
- [x] The header conveys "here's what's happening near {your area}" honestly, using the member's own approximate area; no precise member or venue location is exposed.
- [x] The filtering explanation, if kept, is in plain calm language (no "active blocks" / systems terms) and does not occupy the warmest slot as a disclaimer.
- [x] The results heading reads as human and located (not just a bare count) while stating the real number — no fabricated traction, "people near you", scarcity, streaks, or popularity/attractiveness metrics.
- [x] The first real invitation card sits meaningfully closer to the top than today (reduce the ~542px header-top→first-card gap at 1280) without cramping.
- [x] Exactly one h1; on-brand Ink/Cream/Lime/Sage; AA contrast; visible focus; any greeting motion is reduced-motion-safe (and not fatiguing on a page seen every visit).
- [x] Usable at 1280 and 375 with no horizontal overflow.
- [x] Copy describes only implemented capability; a member returning to an empty feed still gets the warm near-me-aware empty state (do not regress `CX-20260701-discover-no-location-around-me-search`).
- [x] Relevant automated tests (header renders member name + located heading, honest count) and repository checks pass.
...
- 2026-07-02 - Tester: independent retest → **VERIFIED**. Repo checks re-run ONCE (apps/web): typecheck PASS, lint PASS (only the pre-existing `qa/full-flows.mjs` unused-var warning, untracked, not this change), test PASS (452 passed / 12 skipped incl. the new `buildDiscoveryGreeting`/`describeDiscoveryResultsHeading` cases), prod build PASS. LIVE authenticated on pooled Bucharest member (qa-pool-host-a, firstName "Host") via real session cookie: (a) near-me `/discover` → h1 "Welcome back, Host." (exactly one h1 on the page, confirmed by DOM count), sub "Here's what's happening near Bucharest.", results heading "37 invitations near Bucharest" = **37 actual rendered `.discovery-card`s** (count is truthful/dynamic, not hardcoded); (b) `?near=all` → sub "…across every area.", heading "38 invitations everywhere" = 38 cards (correctly surfaces a Cluj event that near-me hides — the located filter is real); (c) typed `?city=Cluj` → **"1 invitation" (singular) = exactly 1 card** proving honest singular/plural + non-`near`/non-`everywhere` third state; (d) typed no-match / `Nowhereville` → warm empty "A quiet court—for now." heading + near-me-aware empty copy intact (empty state correctly bypasses the results helper — not regressed). Area-note is plain calm language ("You're seeing events that fit your sports and age range near Bucharest, your profile area." / "…, everywhere." + a "Search everywhere"/"Back to near Bucharest" toggle) — no "active blocks"/systems jargon, out of the warmest slot. Privacy: greeting uses only the member's own first name + approximate area; cards show approximate area+city (e.g. "Tineretului, Bucharest"), no precise venue. Humane: availability wording honest ("6 places left", "Last place"); no streaks/scores/popularity/"people near you". Fold/responsive (authenticated, reducedMotion:reduce): 1280 header-top→first-card gap = **355px** (down from the prior ~542px, first card `aboveFold:true` in a 900px viewport, two cards visible), no horizontal overflow at 1280 or 375 (`scrollWidth==clientWidth`); mobile 375 stacks header+5-field filter form (pre-existing form, not a regression). Tokens only (--text/--bg/--muted/--lime), focus ring on `.discover-broaden`, no new motion. Note: live browser scripted-login click was flaky, so live verification used a real `/api/auth/login` session (HTTP 200) — a harness artifact, not a product issue; account was NOT rate-limited. Status → `verified` (unstaged for orchestrator to archive).
