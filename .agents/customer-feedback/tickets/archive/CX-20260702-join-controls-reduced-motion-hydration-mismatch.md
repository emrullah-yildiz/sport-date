...
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at HEAD incl. ada3e8d). The fix is correct: `useSyncExternalStore(subscribeNoop, ()=>true, ()=>false)` is the canonical hydration-safe isMounted pattern — getServerSnapshot=false drives both the SSR render AND the first pre-hydration client render, so the `mounted===false` branch renders a bare <div> (no motion props, no inline transform/opacity) → SSR + first client paint byte-identical in every motion setting (confirmed by dumping the real component's server markup — .join-request-box / .join-state render plain, no inline style). AnimatePresence initial={false} suppresses the first-mount entrance so hydration is visually unchanged; later in-place resolutions still fade/rise for motion users; reduced-motion gets instant swaps. Lint-clean (useSyncExternalStore, not setState-in-effect). Checks the Tester ran: typecheck PASS, lint 0 errors, test 570 passed/12 skipped, prod build PASS. FOLLOW-UP (filed separately, not blocking): the added JoinRequestControls.test.tsx regression assertion is effectively a tautology in vitest/jsdom (framer-motion inside AnimatePresence initial={false} emits no inline opacity/transform there regardless of the mount gate, so forcing the bug back on still passes) → CX-20260702-join-controls-hydration-test-is-tautology-strengthen. Fix itself verified sound. Orchestrator archived.
