...
- 2026-07-02 - Implemented by experience-build-agent; status `implemented`; commit `6a026db`. Root cause: `BetaTermPanel` renders block-level markup (`<div className="term-explainer-panel">` wrapping `<ul className="term-explainer-points">`), but the explainer was mounted inside a `<p>` at every render site; opening it injected block elements into a paragraph, so React fired the 4 `<p> cannot contain a nested <ul>/<div>` hydration/nesting errors on client-open (SSR was clean because the panel is closed on first render). Fix: changed the three wrapper elements that host the explainer from `<p>` to `<div>` with the identical class — landing hero (`p.microcopy` → `div.microcopy`, `apps/web/src/app/landing/page.tsx`), login switch CTA (`p.auth-switch` → `div.auth-switch`, `apps/web/src/components/LoginForm.tsx`), and profile eyebrow (`p.eyebrow.eyebrow-with-explainer` → `div`, `apps/web/src/app/profile/page.tsx`). All three render sites of `BetaTermExplainer` were audited; those were the only three and all wrapped it in a `<p>` — all fixed. The `.microcopy`/`.auth-switch`/`.eyebrow` styles are class-based selectors with explicit margins, so the `<div>` renders visually identical at 375 and 1280 (layout unchanged), and the accessible disclosure contract (focusable `<button>` with aria-expanded/aria-controls/aria-describedby, Esc/outside-click/"Got it" close, focus return) and copy are untouched — only the host element changed. No-more-errors confirmed by reasoning (a `<div>` validly contains `<div>`/`<ul>`, so the "`<p>` cannot contain" nesting error can no longer fire) plus a unit regression test in `apps/web/src/components/BetaTermExplainer.test.tsx` asserting (a) the panel is block-level (`<div>` > `<ul>`) and (b) each render site's wrapper hosting `BetaTermExplainer` is a `<div>`, never a `<p>`. Manual check: load `/landing` signed-out, click "What is the private beta?", console has no `<p> cannot contain` error. Checks: typecheck pass, lint pass (only pre-existing warnings in untouched files), test 588 (576 pass / 12 skip), prod build pass. No migration.
- 2026-07-02 - VERIFIED (Tester source, worktree-isolated at HEAD incl. 6a026db; AND user-sim LIVE = 0 console errors, was 4). All 3 BetaTermExplainer render sites now use <div> wrappers (landing/page.tsx:146 .microcopy, LoginForm.tsx:143 .auth-switch, profile/page.tsx:87 .eyebrow-with-explainer); BetaTermPanel renders block <div>>.<ul> which is valid inside <div> → the `<p> cannot contain <ul>/<div>` error is structurally impossible. No visual regression: .microcopy/.eyebrow/.auth-switch/.eyebrow-with-explainer are plain CLASS selectors with explicit margins (no p.microcopy element-qualified rule; only .auth-switch a child selector, unaffected) → div renders identically 375/1280. Disclosure a11y preserved (BetaTermExplainer.tsx internals unchanged: focusable button, aria-expanded/controls/describedby, Esc+focus-return, outside-click, Got-it). Regression test genuine (asserts panel block-level + each render-site wrapper tag == "div", fails if reverted to <p>). Checks the Tester ran: typecheck PASS, lint 0 errors, test 576 passed/12 skipped, prod build PASS. Orchestrator archived.
