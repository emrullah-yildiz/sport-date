...
- [x] When a broadly-rendered server path throws (missing DB URL, missing-migration column error, or any uncaught RSC render error), the member sees a calm, on-brand fallback — Sport Date wordmark, a short human line, AA contrast, no motion issues — not a bare white `NextError`.
- [x] The fallback offers a recovery affordance (a "Try again" action using the boundary `reset`, and/or a link home) with a ≥44px target and visible focus.
- [x] No internal error message, stack, `digest`, SQL, or schema/column name is shown to the member; Sentry capture + `beforeSend` scrubbing is preserved.
- [x] The fallback is self-contained (does not itself depend on a data fetch that could throw) since `global-error.tsx` replaces the root layout.
- [x] Layout is usable and overflow-free at 375px and 1280px.
- [x] Relevant automated tests and repository checks pass (add a render test asserting the calm fallback shows the wordmark + recovery action and does NOT render the raw error text).
...
- 2026-07-01 - Experience Build Agent implemented (commit `d154394`, LOCAL only — orchestrator is holding pushes while prod DB is behind migrations). Rewrote `apps/web/src/app/global-error.tsx` to render a calm, self-contained, on-brand fallback: Sport Date wordmark, honest reassuring copy ("Something went wrong on our end — your account is safe. Please try again in a moment."), a "Try again" action wired to the boundary `unstable_retry` prop (this Next 16.2 signature; `reset` still exists but the docs/repo steer to `unstable_retry`, e.g. `hosting/error.tsx`), and a link to a surviving page (`/login`). All styles inlined because `global-error` replaces the root layout (globals.css unavailable). Brand Ink/Cream/Lime with a single thin Coral top-accent; `role="main"` + `role="alert"`, focusable `h1` (`tabindex=-1`), ≥44px targets, visible focus, reduced-motion parity, AA contrast, overflow-free at 375px/1280px. No internal error text/stack/`digest`/SQL/column shown; `Sentry.captureException` + shared `beforeSend` PII scrub preserved; no false "we've been notified" claim. Added `global-error.test.tsx` (renderToStaticMarkup) asserting wordmark + recovery action present and raw error internals absent. Checks: typecheck pass, lint pass (only the pre-existing untracked qa `full-flows.mjs` warns — not touched), test 274 pass, **production `npm run build --workspace @sport-date/web` pass**. Dev-server sanity check via a temporary throwing scratch route confirmed the boundary triggers (dev shows Next's own overlay on top, expected dev-only) and the wordmark/no-leak held; scratch route reverted. Follow-up worth filing: align route-level `hosting/error.tsx` + `events/[eventId]/room/error.tsx` and add a shared calm-error component so the branded fallback is reused. Left for independent retest by the Explorer.
- 2026-07-01 - **Verified** by Tester (source + build + render-test; the live-500 path is intentionally not induced on a running app). Read `apps/web/src/app/global-error.tsx`: renders its own `<html><body>` fully inline-styled (globals.css unavailable here) with the "Sport Date" wordmark, calm h1 "Something went wrong on our end", honest body ("Your account is safe … try again in a moment"), a lime "Try again" button wired to the `unstable_retry` boundary prop (falls back to `window.location.reload()` when absent), and a "Go to sign in" → `/login` link. Both actions are `min-height:44`, `:focus-visible` 3px ink outline; a single thin coral top accent; `role="main"` + inner `role="alert"`, focusable h1 (`tabIndex=-1`); `@media (prefers-reduced-motion: reduce)` removes the button transition (reduced-motion parity); `<meta robots noindex>`. NOTHING from `error` (message/stack/`digest`/SQL/column) is rendered — only `Sentry.captureException(error)` in an effect, preserving the shared `beforeSend` scrub; no false "we've been notified" claim. Fully self-contained (no data fetch). `global-error.test.tsx` present. Palette is Ink/Cream/Lime with AA-legible dark-on-light text. Repo checks: typecheck/lint pass, test 319 pass/12 skip, **production build pass**. All criteria met. (Follow-up the builder flagged — a shared calm-error component reused by `hosting/error.tsx` + `events/[eventId]/room/error.tsx` — remains open scope, noted for Planner, not part of this ticket.)
