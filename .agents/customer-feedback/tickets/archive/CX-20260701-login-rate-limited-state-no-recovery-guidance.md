...
- 2026-07-01 - Independent retest by experience-design-explorer; status `implemented` → **`verified`**. Retested from the member scenario at source + unit + non-429 live (per the hardened handshake — live 429 is IP-limited, not polled). (1) 429 recovery: `interpretLoginFailure` reads the server `Retry-After`, clamps ≤30min, and `formatRateLimitMessage` renders a calm "you can try again in about N minutes" — a unit test asserts it never dumps raw seconds — AC1 ✅. (2) Cool-down disables submit: `LoginForm` `submitDisabled = submitting || lockedOut` with `lockedOut = cooldownSeconds > 0`, button reads "Please wait…", a 1s interval counts down and re-enables at 0 — AC2 ✅. (3) Recovery route: on `suggestRecovery` the form opens the controllable `ForgotPasswordPanel` (`open`/`onOpenChange`) and shows the separate-limit hint — AC3 ✅. (4) a11y: `role="alert"` on the error block, `aria-hidden` mm:ss countdown (no per-second AT chatter), btn `min-height:52px` (>44px), `.auth-cooldown`/`.auth-recovery-hint` `#435649` on cream (AA), no animation → reduced-motion-safe — AC4/AC6 ✅. (5) Anti-enumeration: unit test confirms 401 "Email or password is incorrect." passes through verbatim with `cooldownSeconds:0`/`suggestRecovery:false`, and a 401 carrying a stray Retry-After does NOT leak a cool-down; limiter/thresholds untouched — AC5 ✅. Live non-429 login (pooled `seeker-B`) → 200 + `auth_token` session cookie set, confirming the happy path is intact. Unit: `login-recovery.test.ts` 12/12 (+`join-request-policy` 6/6) = 18/18 pass. Live 429 not reproduced (IP-limited — not polled per policy); every 429-branch criterion is covered by the 12 unit tests + source, and the non-429 path was observed live. All criteria met → `verified`.
- 2026-07-01 - Implemented by experience-build-agent; status `implemented` (commit `3d9b32a`). New pure lib `apps/web/src/lib/login-recovery.ts` parses the login response: on 429 it reads the server's `Retry-After` (already sent by `enforceRateLimit`; same-origin fetch exposes it — no API change needed), clamps it to <=30 min, and renders a calm blame-free "Too many attempts — you can try again in about N minutes." `LoginForm.tsx` now runs a cool-down that disables submit (button reads "Please wait…") so re-clicks can't extend the lockout, shows a visual-only (`aria-hidden`) mm:ss countdown so AT gets the calm minute-level message without per-second chatter, and opens `ForgotPasswordPanel` (made controllable via optional `open`/`onOpenChange`) with a recovery hint noting reset uses a separate limit. `role="alert"` retained on the error block, no focus trap, 44px+ targets, AA contrast (error `#8f2a2a` on coral tint; supporting copy `#435649`), reduced-motion safe (no animation). Anti-enumeration untouched: non-429 failures (incl. the generic 401) pass the server copy through verbatim; limiter/thresholds unchanged. Checks: typecheck pass; lint pass (only the pre-existing untracked `qa/full-flows.mjs` warning remains); test pass 189 passed/12 skipped, +12 new `login-recovery.test.ts` (retry-after delta+date parsing, clamping, calm copy, 429->cooldown+recovery, 401 pass-through, no-Retry-After-leak on non-429). Prod build of `/login` OK; live non-rate-limited login (pooled `seeker-D`) -> 200 + session cookie; wrong-credentials -> generic 401. Secondary `window.location.assign` hard-nav on success left as-is (out of scope; own ticket). Live 429 not reproduced (IP-limited) per policy — covered by unit + source verification.
