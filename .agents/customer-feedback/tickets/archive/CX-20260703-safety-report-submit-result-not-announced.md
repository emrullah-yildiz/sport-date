...
- 2026-07-03 - Independently verified by Tester (source + checks, no live SR this pass). Read commit `2da6464`: all four forms (`ReportSafetyControls`, `SafetyAppealForm`, `ModerationCaseForm`, `ModerationAppealForm`) now render two always-mounted region `<div>`s — outer `role="status" aria-live="polite"` (success) and outer `role="alert" aria-live="assertive"` (failure), present & empty before submit with a conditional inner `<p>`; this is the correct WCAG 4.1.3 persistent-region shape and matches the `RoomLeaveControl`/`EditProfileForm` reference. AC1 ✓ (persistent region on `ReportSafetyControls`), AC2 ✓ (failure assertive, copy unchanged generic error), AC3 ✓ (identical treatment on all four — consistent), AC4 ✓ (no-block report success routes `announce(msg,"success",!blockUser)` → `requestAnimationFrame` focuses the `<p ref tabIndex=-1>` confirmation), AC5 ✓ (`aria-busy={submitting}` on all four submit buttons). Guardrails ✓: emergency-services reminder still present/reachable in `ReportSafetyControls`; no safety capability paywalled/removed; error copy unchanged. Coral `.safety-message-error` uses `var(--coral)` sparingly (left border + faint bg); empty regions add no visible chrome. New tests (7, +7) genuinely fail against the OLD mount-on-message code (old code rendered nothing at idle, so `role="status"`/`alert`/`aria-live`/`aria-busy` assertions distinguish new from old) — not tautological, though they only assert presence, not tone-routing or focus-move. Checks: typecheck ✓, lint 0 errors (2 pre-existing unrelated warnings), test 735 passed/12 skipped (747), build ✓. No db migration added. Residual: real screen-reader end-to-end announcement of an already-mounted region still best confirmed with a live reader/dev server; `SafetyAppealForm` success is pre-empted by `window.location.reload()` (accepted — reloaded page IS the state change and its assertive FAILURE path does not reload and is announced); empty regions inside the grid `.moderation-case-form` add ~12px idle whitespace (cosmetic, no overflow). Status `verified`.
