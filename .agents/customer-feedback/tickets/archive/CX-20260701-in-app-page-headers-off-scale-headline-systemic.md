...
- `.auth-card h1` (line ~447) — `clamp(34px,7vw,48px)/line-height:1/-.055em` — **added 2026-07-01 (verify-email × accessibility/state-coverage pass, live-measured)**. This ONE selector governs the whole unauthenticated / account-security entry family: `/login`, `/verify-email`, `/reset-password`, and the password-reset-confirm card all render their h1 through `.auth-card h1`. Live-measured at 1280px: 48px / line-height 48px (=1) / letter-spacing -2.64px (−.055em); at 375px: 34px / -1.87px — off the `--fs-h1` token (which would resolve to 60px at 1280 with the systemic -.02em/1.05) and *tighter still* than the in-app off-scale clamps (line-height 1 with -.055em vs .95/-.065em). Confirmed live: `/login` h1 "Pick up where the movement left you." and `/verify-email` h1 render at 48px/-2.64px. (Note: `/signup` uses a *different*, non-`.auth-card` header — 36px/-1.08px at 1280 — so it is out of this selector's scope and is tracked separately under the signup double-headline ticket.)
...
- [ ] The shared `.auth-card h1` (used by `/login`, `/verify-email`, `/reset-password`, and the password-reset-confirm card) also uses the shared `--fs-h1` token and systemic `-.02em`/`1.05` treatment — no bespoke `clamp(34px,7vw,48px)/line-height:1/-.055em`. The auth cards are the member's FIRST impression of the type system (first screen before signing in), so consistency there carries extra trust weight. Long login/verify headings still wrap cleanly with no clipping of descenders at line-height 1.05 (the current line-height:1 risks tight descenders).
...
- 2026-07-02 - Builder (systemic typography build pass) → Status `implemented`. VERIFIED ALREADY DELIVERED, no new code change needed (re-implementing would be a no-op). The systemic inconsistency this ticket describes — every in-app page h1 hard-coding a bespoke off-scale `clamp(48px,7vw,82–86px)` with `-.065em`/`.95` instead of the shared `--fs-h1` token — was fully resolved by the superseding ticket's commit `d3772fe` ("style: right-size headings and calm the type scale across the app — CX-20260702-typography-right-size-and-scale"). Confirmed via source + git: all 12 bespoke clamps are deleted from `apps/web/src/app/globals.css`; `grep` finds ZERO remaining `font-size: clamp(...)` heading rules in globals.css. THE SHARED SCALE APPLIED: `:root` now defines `--fs-display: clamp(40px,5vw,60px)` (marketing hero ONLY), `--fs-h1: clamp(30px,3.4vw,42px)` (every in-app page header), `--fs-h2: clamp(24px,2.4vw,30px)`; the base `h1,h2,h3,h4 { letter-spacing:-.02em; line-height:1.1 }` rule supplies the systemic treatment (no per-surface `-.065em`/`.95`). PER-PAGE OVERRIDES CONSOLIDATED: `.discover-header h1`, `.hosting-header h1`, `.safety-center-header h1`, `.new-event-header h1/.host-event-hero h1`, `.event-detail-hero h1`, `.room-hero h1`, `.moderation-header h1`, `.legal-hero h1`, `.feedback-header h1`, and `.auth-card h1` (login/verify-email/reset-password/reset-confirm family) all now read `font-size: var(--fs-h1); line-height: 1.1` with no bespoke clamp/tracking; `.hero-title` reads `var(--fs-display)` and `.steps h2/.safety h2` → `var(--fs-h2)` via `.section-title`. `/profile` (`.profile-hero h1`) already matched and still does. The only remaining bespoke clamps live in `research/bucharest/page.module.css`, an out-of-scope marketing/research artifact (design-refresh §3.5), scoped to that one route — it does not fight the in-app scale. COMPUTED SIZES I MEASURED LIVE (Playwright, logged in as qa-pool-host-a, computed `getComputedStyle`): at **375px** — /discover, /hosting, /safety, /events/new, /feedback, /profile ALL h1 = 30px / line-height 33px (1.1) / letter-spacing −0.6px (−.02em) / 0px horizontal overflow; at **1280px** — same six pages ALL h1 = 42px / line-height 46.2px (1.1) / letter-spacing −0.84px (−.02em) / 0px horizontal overflow. Perfectly consistent across all six surfaces at both widths, identical to `/profile`, no page overflows, headings wrap cleanly. AA: unchanged — this is size/tracking-only; heading semantics (single h1, h1→h2 order), focus, and reduced-motion untouched; palette AA per design-refresh §1 measured table. TESTS: no unit change — the scale is pure CSS-token consolidation with no component/prop seam to assert (headers are className-driven CSS, not a shared page-header component); relied on the full suite (typecheck ✓, lint ✓ 0 errors, 728 tests ✓ incl ethical-energy-guardrails, `npm run build` ✓) plus the multi-page computed-size measurements above. COMMIT: no new commit for this ticket (work already in `d3772fe`); this handoff + status edit are the only change and are left unstaged per protocol. `.gitignore` shows a pre-existing unrelated working-tree edit — left untouched.
- 2026-07-01 - Extended by Experience & Design Explorer (verify-email × accessibility / state-coverage pass). Added the shared `.auth-card h1` selector (line ~447) to the scope after live-measuring it at 48px/-2.64px (1280) & 34px/-1.87px (375) on `/login` and `/verify-email`. This one selector reaches the whole unauthenticated / account-security entry family (`/login`, `/verify-email`, `/reset-password`, password-reset-confirm) — the member's first impression of the type system — so folding it into this reconciliation raises Reach without changing the P3 severity (still visual-fidelity only, no functional/a11y floor). Added a matching acceptance-criterion line + a descender note (current line-height:1 vs systemic 1.05). No dedicated new ticket filed: this is the same defect class as the eight in-app headers already tracked here, so per the dedupe rule it is linked/extended rather than duplicated.
