...
- [x] When the automatic verification result resolves (verified / already_verified / expired / unavailable / error), the outcome is announced to assistive technology via a live region (`role="status"`/`aria-live="polite"`, or `role="alert"` for the hard-error branch) and/or keyboard focus is moved to the resolved heading — a member using a screen reader is told the result without manually re-reading the page. — VERIFIED source+unit (both a persistent live region AND a focus move on resolve).
- [x] The pending state is exposed programmatically while confirming (e.g. `aria-busy` and/or the "Checking secure token…" text as a `role="status"`), so "verifying" is announced, not only shown. — `aria-busy={isLoading}` on the card + `role="status"` on the pending line; unit-asserted.
- [x] A focus move (if used) does not trap the keyboard and there is no input on the card to steal focus from; tab order remains sensible and the forward action links stay reachable. — focus fires ONLY on async resolve (`focusOnResolveRef`), never on plain load; no input; forward links reachable.
- [x] Anti-enumeration preserved: announced copy never reveals whether an account/email exists; the generic "invalid" copy is retained. — unit asserts absence of account-existence signals.
- [x] The honest disclaimer ("confirms inbox access only… does not verify identity, age, location accuracy, or in-person safety") is preserved verbatim; no manufactured urgency or dark patterns; token validation/expiry unchanged. — present verbatim; `isBrowserEmailVerificationToken` gate unchanged.
- [x] Layout stays usable and overflow-free at 375px and 1280px; forward links keep ≥44px touch targets and visible focus; contrast unchanged (h1 stays dark ink on white). — no layout change; `btn-primary`/`btn-secondary` links unchanged.
- [x] Relevant automated tests and repository checks pass (add a unit test asserting the resolved state renders inside a live region / receives focus for at least the verified and expired branches). — `EmailVerificationConfirmCard.test.tsx` 9/9 pass; production build pass.
...
- 2026-07-01 - Independently retested by Experience & Design Explorer (source + unit, resolved in ONE step per the no-poll handshake — live token delivery is not available in this env, so the async verified/already/expired/unavailable/error branches are source-verified, not driven by real AT). Result: VERIFIED. Confirmed at `apps/web/src/components/EmailVerificationConfirmCard.tsx` WITHOUT the async branches needing a live token: the outcome heading+body are wrapped in a PERSISTENT live region — `role="status"`/`aria-live="polite"` for verified/already_verified/expired, `role="alert"`/`aria-live="assertive"` for the hard-error branch (503 unavailable / could-not-finish) — so the result is announced in place on resolve; the card is `aria-busy={isLoading}` while the confirm POST is pending and the "Checking secure token..." line is itself `role="status"`; focus moves to the resolved heading ONLY on the async resolve via `focusOnResolveRef` (set true inside the confirm handler, never on the synchronous missing/invalid initial paint) + the `attachHeading` callback ref that calls `node.focus()` — no focus theft on a plain load and no keyboard trap (the card has no input; forward `btn-primary`/`btn-secondary` links stay reachable). Anti-enumeration preserved (generic "invalid" copy, no account-existence signal — unit asserts absence of `account exists|no account|already registered|not registered`); the honest disclaimer ("confirms inbox access only… does not verify identity, age, location accuracy, or in-person safety") is present verbatim; token validation/expiry (`isBrowserEmailVerificationToken`) unchanged. `EmailVerificationConfirmCard.test.tsx` 9/9 pass — asserting the polite live region + aria-busy + focusable `<h1 tabindex="-1">` on the pending/synchronous states AND the resolved verified/expired branches in a polite region + the unavailable/error branches announced assertively, each with a focusable heading. Production build passed earlier this pass. Only source-only sub-branch: the real end-to-end async announcement with live assistive tech + a real/expired token (delivery not live in this env) — structurally covered because the live region + focus move are unconditional on every resolved branch. All acceptance criteria met → `verified`.
- 2026-07-01 - Experience Build Agent (Claude Opus 4.8) implemented; status `implemented`, commit `cf02e96`. Wrapped the outcome heading/body in a persistent live region (`role="status"`/`aria-live="polite"` for verified/already_verified/expired; `role="alert"`/`aria-live="assertive"` for the hard-error branch), marked the card `aria-busy` while confirming and gave the "Checking secure token" line `role="status"`, and moved keyboard focus to the resolved heading (`tabIndex={-1}` + callback ref) only on the async resolve — never on a plain load, no keyboard trap. Anti-enumeration copy, token validation/expiry, the honest disclaimer, 44px links, and all visual states unchanged. Split out `VerificationCardBody` so the resolved outcomes are unit-testable; added a test asserting the verified/expired branches render in a polite live region with a focusable heading and the hard-error branches announce assertively. Checks (all `--workspace @sport-date/web`): typecheck pass, lint pass (0 errors), test pass (256 passed / 12 skipped, +9 new), production build pass (Compiled successfully). Dev server confirmed the live region + focusable heading + `aria-busy` render on `/verify-email`. Committed LOCALLY only (orchestrator holding pushes). Ready for independent retest with real assistive tech across the async branches.
