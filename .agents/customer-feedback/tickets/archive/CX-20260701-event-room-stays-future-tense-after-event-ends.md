...
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. 9cb2801). All 6 pass. (1) every shift is a room.hasEnded ternary — hero eyebrow (after the game vs coordination room), hero cue (This event has finished.{date} vs bare date), venue (Where you met vs Where you are meeting), people (Who had a place vs Who has a place; N had a place vs N joining); pre-event copy is the else side → upcoming room byte-for-byte intact. (2) non-punitive: ended-host-zero → "No one joined this time" (future "Your event is live"/"No one has a place yet" suppressed via hostAwaitingFirstPlace && !hasEnded); sole-guest "first to join" suppressed when ended; test asserts absence of failed/no-show/streak/score. (3) afterglow leads: .room-rhythm "A calm arrival" wrapped {hasEnded ? null : ...}; PostEventAfterglow→ShareMotivationalCard→EventReflectionForm→PeerFeedbackPanel render only when ended; PreArrivalSafetyBrief/FirstEventPreparationCard remain !hasEnded — no regression. (4) privacy/auth intact: diff only labels/tense + rhythm wrapper; venueName/address + accepted-member/host gating unchanged; peer-feedback gate intact; no precise location added. (5) .room-hero-ended uses var(--lime) only, no hex, 9px decorative ::before dot, no new motion. (6) 7 tests assert new past-tense strings AND absence of future-tense copy + pre-event no-regression. Checks the Tester ran: typecheck PASS, lint 0 errors, test 651 passed/12 skipped (room 25 incl 7 ended-room), prod build PASS. Orchestrator archived.
