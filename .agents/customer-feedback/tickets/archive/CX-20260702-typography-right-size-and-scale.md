...
- 2026-07-02 - test - **VERIFIED** (independent retest, one step). Repo checks all pass. Source-confirmed the shared scale in `globals.css`: `--fs-display` clamp(40,5vw,60) (landing only), `--fs-h1` clamp(30,3.4vw,42) (every in-app h1), `--fs-h2` clamp(24,2.4vw,30), `--fs-h3` 20, `--fs-body` 16, `--fs-small` 14, `--fs-label` 12 — exactly the target scale. A regex sweep found NO remaining bespoke off-scale headline clamp (78–92px) and no per-surface `-.06/-.065/-.075em` override (only comments documenting their removal). Matches the prior live measurement (h1 42px@1280 / 30px@375, -.02em, 1.1). Known pre-existing `.profile-photos-upload-label` @375 overflow is a component control, not typography. Status `implemented` → `verified`.
- 2026-07-02 - Implemented (commit `d3772fe`). Replaced all bespoke off-scale headline clamps in `globals.css` with the shared scale: `--fs-display` (landing hero only), `--fs-h1` for every in-app page h1 (discover/hosting/safety/event-detail/room/events-new/feedback/moderation/legal/signup/auth) with systemic -.02em/1.1; dropped oversized sub-headings to `--fs-h2`/`--fs-h3` and capped big counts (movement/received-rating/discovery day-time) at `--fs-h1`/`--fs-h3`; added `--space-1..6` + `--space-heading-gap` and applied to heading→sub-text gaps (absorbs the vertical-rhythm ticket); `--fs-body` 17→16, added `--fs-label`. Theme/contrast tokens, focus, 44px targets, reduced-motion unchanged; copy meaning unchanged. Verified live signed-in (pooled host-A) at 1280 & 375 across landing/discover/profile/hosting/events-new/safety/feedback/login: every in-app h1 = 42px/30px, -.02em, 1.1; only landing on `--fs-display`; no header overflow. (One pre-existing profile @375 overflow traced to `.profile-photos-upload-label` file input — a component control, unrelated to typography, not introduced here.) Checks: typecheck ✓, lint ✓, test 400 passed/12 skipped ✓, production build ✓. No migration. Status `implemented` — awaiting independent retest by the Explorer.
