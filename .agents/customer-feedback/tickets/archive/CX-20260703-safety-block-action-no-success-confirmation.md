...
- 2026-07-03 - Independently verified by orchestrator (source + repo checks): standalone `blockOnly()` success now latches a `blocked` state and routes through the P1 persistent polite live region with focus-move to the confirmation (`announce(blockConfirmationMessage(...), "success", true)`) — the silent bare `window.location.assign("/profile")` is gone from the block-only path (the sole remaining redirect is the intended report+block path, `if (blockUser)`). Button cycles Block→Blocking…→Blocked + `disabled`/`aria-busy`; failure stays assertive; P1 regions intact; confirmation copy is calm/factual, names what the block prevents, points to manage-blocks, leaks no data beyond the display name. typecheck/lint/744 tests/prod build pass (commit d313965). Status `verified`.
