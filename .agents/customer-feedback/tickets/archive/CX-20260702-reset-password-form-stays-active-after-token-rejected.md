...
- `2026-07-02` - Implemented by Builder (commit `b2744b0`). Added a `tokenRejectedByServer` state to `PasswordResetConfirmCard.tsx`, set only when the confirm POST returns state `invalid` (400) or `expired` (410) — via the new pure `isServerTokenRejection` helper. The "Update password" form now renders only when the token is format-valid AND not server-rejected AND not completed (`showPasswordForm`), so on server token-rejection the form is removed and only the error + inline "Send me a new reset link" recovery panel remain — a member can no longer re-submit against a dead token. A client-side password mismatch and a transient 5xx (`unavailable`/`error`) do NOT hide the form (token may still be good; member keeps it to correct/retry). On rejection, keyboard focus moves to a focusable `role="alert"` region (`tabIndex={-1}`, `.auth-flow-recovery`) wrapping the error + recovery panel (mirrors the verified `EmailVerificationConfirmCard` focus-on-resolve callback-ref pattern) instead of dropping to `<body>`. Preserved: no-token/malformed dead-ends (unchanged), anti-enumeration (same `ForgotPasswordPanel` request path), token validation/expiry (no security change), "Back to sign in", and the valid-token success flow. Split out a presentational `PasswordResetConfirmBody` (like `VerificationCardBody`) so the resolved states are statically testable; added tests for server-rejection hides-form + recovery-panel + focusable alert region, client-mismatch keeps-form, valid-token completes, and the client-vs-server distinction helper. No globals.css/token change. No migration. Checks (apps/web): typecheck pass, lint pass (0 errors), test pass (595 passed / 12 skipped), `npm run build` pass. Handoff for independent retest.
- 2026-07-02 - VERIFIED (Tester, independent, worktree-isolated at clean HEAD incl. b2744b0). All 7 properties pass. (1) form hides on dead token: gate `showPasswordForm = !completed && tokenValid && !tokenRejectedByServer`; on invalid(400)/expired(410) only error + ForgotPasswordPanel render inside role=alert; body test asserts #reset-password/"Update password" absent + recovery panel present → no re-submit against dead token. (2) client mismatch keeps form: handleSubmit returns early on validatePasswordResetDraft failure BEFORE fetch, never sets tokenRejectedByServer. (3) transient 5xx keeps form: isServerTokenRejection true ONLY for invalid/expired, false for validation_error/unavailable/error/undefined/ok — matches confirm route contract (route.test.ts verifies). (4) focus moves: focusOnRejectRef + attachRejectionRegion callback ref .focus() on tabIndex=-1 .auth-flow-recovery role=alert node (mirrors EmailVerificationConfirmCard); not dropped to body. (5) no security regression: token validation/expiry/CSRF-403/rate-limit/JSON-400 untouched; recovery same anti-enumeration ForgotPasswordPanel/password-reset/request; no-token/malformed + valid-token flows unchanged. (6) 13 card tests + route-branch coverage assert real DOM presence/absence + helper truth table, not tautologies. Checks the Tester ran: typecheck PASS, lint 0 errors, test 595 passed/12 skipped (targeted 20 passed), prod build PASS. Orchestrator archived.
