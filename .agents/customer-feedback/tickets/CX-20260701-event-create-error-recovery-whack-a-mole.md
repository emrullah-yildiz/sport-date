...
- 2026-07-01 - Experience & Design Explorer: independently retested from the member scenario WITHOUT reading the diff first, live in Chromium (reduced-motion) as pooled `host-A` (single login, reused). VERIFIED — every AC passes: (1) `/events/new` renders `min="2026-07-01T21:15"` on the start-time `datetime-local`, so a past time cannot be picked client-side. (2) With required fields empty, clicking "Publish the invitation" (after scrolling to the bottom of the form) shows a VISIBLE `role=alert` summary at the TOP, headline "A few required details are still empty below — please complete them to publish", listing all 10 missing fields as anchor links, PLUS a `role=status` action-alert just above Publish ("Some required details above still need attention — we've highlighted them for you"). (3) Focus MOVED to the first invalid field (`#sport`) and it was scrolled INTO the viewport (in-view=true) — not left at the bottom, not just a native bubble. (4) All 10 empty fields carry `aria-invalid=true` with inline `#<field>-error.field-error` messages present. (5) A server rejection (past start + short description via POST /api/events) returns the FULL `errors[]` array = ["Event description must contain 20 to 1000 characters.","Event must start in the future."] (2 errors, field-mappable), not just errors[0]. (6) A complete, valid form published and landed on `/events/3500db84-…?published=1`. No overflow at 375. Status `verified`.
- 2026-07-01 - User-simulator (HOST → COORDINATE journey drive): RE-CONFIRMED LIVE as pooled `host-A`. Empty-form publish showed the top `role=alert` summary "A few required details are still empty below — please complete them to publish" with the count "10 required details still need your attention:" and all 10 fields listed; focus moved to `#sport` (focusInView=true); 10 elements carried `aria-invalid=true`. A fully-filled form (Tennis / description ≥20 chars / near-future start / language / public area + private venue) published and landed on `/events/{id}?published=1` with the "IT'S LIVE" success block. Every checked AC holds live — no regression. (Owner's original "Publish appears to do nothing" symptom does not reproduce.)
